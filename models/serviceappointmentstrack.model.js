module.exports = mongoose => {
    ...
    const ServiceAppointmentsTrack = mongoose.model("serviceappointmentstracks", schema);
    
    ServiceAppointmentsTrack.getDocumentsByDate = function(date){
    ...
    ServiceAppointmentsTrack.getMinimumDate = function(){
        return this.find().sort({latestOperationDate:1}).limit(1)
    }

    ServiceAppointmentsTrack.cleanup = function(date){
        var start = new Date(date.setUTCHours(0, 0, 0, 0));
        var end = new Date(date.setUTCHours(23, 59, 59, 999));
    ...
    return ServiceAppointmentsTrack;
};